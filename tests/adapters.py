...
from cs336_basics.regex_bpe_tokenizer import train, Tokenizer
from cs336_basics.transformer_lm import (
                                        Linear,
                                        Embedding,
                                        CustomRMSNorm,
                                        PositionWiseFFN,
                                        RotaryPositionalEmbeddding,
                                        TransformerBlock,
                                        TransformerLM,
                                        customSiLU,
                                        customSoftmax,
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    model = CausalMHSAwithRoPE(d_model, num_heads, theta, max_seq_len)
    model.load_state_dict({
        "Wq.W": q_proj_weight,
        "Wk.W": k_proj_weight,
        "Wv.W": v_proj_weight,
    ...
    x = model(in_features, token_positions)
    return x
    # raise NotImplementedError
...
def run_rope(
    ...
    """
    model = RotaryPositionalEmbeddding(theta, d_k, max_seq_len)
    x = model(in_query_or_key, token_positions)
    return x
    # raise NotImplementedError
...
def run_transformer_block(
    ...
    model.attention.load_state_dict({
        "Wq.W": weights["attn.q_proj.weight"],
        "Wk.W": weights["attn.k_proj.weight"],
        "Wv.W": weights["attn.v_proj.weight"],
        "Wo.W": weights["attn.output_proj.weight"]
    })

    model.pw_ffn.load_state_dict({
        "W1": weights["ffn.w1.weight"],
        "W2": weights["ffn.w2.weight"],
        "W3": weights["ffn.w3.weight"]
    })

    model.l1s1_rms_norm.load_state_dict({
        "gain": weights["ln1.weight"]
    })

    model.l1s2_rms_norm.load_state_dict({
        "gain": weights["ln2.weight"]
    })

    x = model.forward(in_features)
    return x
    # raise NotImplementedError
...
def run_transformer_lm(
    ...
    """
    model = TransformerLM(d_model, vocab_size, context_length, num_layers, num_heads, d_ff, rope_theta)
    model.embeddings.load_state_dict({"embedding_matrix": weights["token_embeddings.weight"]})
    for i in range(num_layers):
        model.transformer_layers[i].attention.load_state_dict({
            "Wq.W": weights[f"layers.{i}.attn.q_proj.weight"],
            "Wk.W": weights[f"layers.{i}.attn.k_proj.weight"],
            "Wv.W": weights[f"layers.{i}.attn.v_proj.weight"],
            "Wo.W": weights[f"layers.{i}.attn.output_proj.weight"],
        })
        model.transformer_layers[i].l1s1_rms_norm.load_state_dict({
            "gain": weights[f"layers.{i}.ln1.weight"]
        })

        model.transformer_layers[i].l1s2_rms_norm.load_state_dict({
            "gain": weights[f"layers.{i}.ln2.weight"]
        })

        model.transformer_layers[i].pw_ffn.load_state_dict({
            "W1": weights[f"layers.{i}.ffn.w1.weight"],
            "W2": weights[f"layers.{i}.ffn.w2.weight"],
            "W3": weights[f"layers.{i}.ffn.w3.weight"]
        })

    model.rms_norm.load_state_dict({
...
def run_rmsnorm(
    ...
    """
    model = CustomRMSNorm(d_model, eps)
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    x = customSoftmax(in_features, dim)
    return x
    # raise NotImplementedError
